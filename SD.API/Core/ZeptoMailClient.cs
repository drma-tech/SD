...
namespace SD.API.Core
{
    public class ZeptoMailClient(IHttpClientFactory factory, string apiKey)
    {
        ...
        {
            name = name.NotEmpty() ? name : address.Split("@")[0];
        ...
        public async Task SendFollowingTemplate(string user_id, string address, string? name, string? franchises, string? series, CancellationToken cancellationToken)
        {
            name ??= address.Split("@")[0];

            var payload = new
            {
                template_alias = "following",
                from = new { address = noreplyEmail, name = appName },
                to = new[] { new { email_address = new { address, name } } },
                merge_info = new
                {
        ...
        public async Task SendInactiveTemplate(string user_id, string address, string? name, CancellationToken cancellationToken)
        {
            name ??= address.Split("@")[0];

            var payload = new
            {
                template_alias = "inactive",
                from = new { address = noreplyEmail, name = appName },
                to = new[] { new { email_address = new { address, name } } },
                merge_info = new
                {
